...
export async function deleteNote(user_id: string, id: number) {
  noStore();
  try {
    await sql.query<Note>(`DELETE FROM ${get_user_table_name(user_id)} WHERE id = $1`, [id]);
  } catch (error) {
    console.error('Database Error:', error);
...
export async function createNote(user_id: string, note: { title: string; content: string }) {
  noStore();
  try {
    const newNote = await sql.query<Note>(`
      INSERT INTO ${get_user_table_name(user_id)} (title, content)
      VALUES ($1, $2)
      RETURNING *`, [note.title, note.content]);
    return newNote.rows;
  } catch (error) {
    console.error('Database Error:', error);
